...
class ResnetPeriodEstimator(Model):
  """RepNet model."""
  ...
  def call(self, x):
    ...
    batch_size = tf.shape(x)[0] #assum batch_size = 20 , x = (20, 64, 112, 112, 3)
    # Conv Feature Extractor.
    x = tf.reshape(x, [-1, self.image_size, self.image_size, 3]) # x = (1280,112,112,3)
    x = self.base_model(x) # x = (1280, 7, 7, 1024)
    h = tf.shape(x)[1]
    w = tf.shape(x)[2]
    c = tf.shape(x)[3]
    x = tf.reshape(x, [batch_size, -1, h, w, c]) # x = (20, 64, 7, 7, 1024)
    ...
    for bn_layer, conv_layer in zip(self.temporal_bn_layers,
      ...
      x = tf.nn.relu(x)
                                 # x = (20, 64, 7, 7, 512)
    x = tf.reduce_max(x, [2, 3]) # x = (20, 64, 512)

    # Reshape and prepare embs for output.
    final_embs = x

    # Get self-similarity matrix.
    x = get_sims(x, self.temperature) # x = (20, 64, 64, 1), 相似度矩阵 64*64
    final_sim = x

    # 3x3 conv layer on self-similarity matrix.
    x = self.conv_3x3_layer(x) # x = (20, 64, 64, 32)
    x = tf.reshape(x, [batch_size, self.num_frames, -1]) # x = (20, 64, 2048)
    within_period_x = x # within_period_x = x

    # Period prediction.
    x = self.input_projection(x) # x = (20, 64, 512)
    x += self.pos_encoding # x = (20, 64, 512)
    for transformer_layer in self.transformer_layers:
      x = transformer_layer(x)                        # x = (20, 64, 512)
    x = flatten_sequential_feats(x, batch_size, self.num_frames) # x = (20, 64, 512)
    for fc_layer in self.fc_layers: 
      x = self.dropout_layer(x)
      x = fc_layer(x)   # x = (20, 64, 32)

    # Within period prediction.
    within_period_x = self.input_projection2(within_period_x) # within_period_x = (20, 64, 512)
    within_period_x += self.pos_encoding2 # within_period_x = (20, 64, 512)
    for transformer_layer in self.transformer_layers2:
      within_period_x = transformer_layer(within_period_x)  # within_period_x = (20, 64, 512)
    within_period_x = flatten_sequential_feats(within_period_x,
                                               batch_size,
                                               self.num_frames) # within_period_x = (20, 64, 512)
    for fc_layer in self.within_period_fc_layers:
      ...
      within_period_x = fc_layer(within_period_x)
                                                                # within_period_x = (20, 64, 1)
    return x, within_period_x, final_sim

  # def build_graph(self, input_shape):
  #   input = Input(shape=input_shape,batch_size=1,dtype=float32)
